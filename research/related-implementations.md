...
| P15 | [Fabio Yáñez Romero: *Your Second Brain Doesn’t Need RAG. It Needs a Map*](karpathy-markdown-vault-vs-hierarchical-maps.md) | Breaking limits of Karpathy flat markdown vaults; 2D Map Architecture (`card.md` vs `full.md`), line-interval section maps, coordinate navigation | Full article & vault paper citations read |
| P16 | [Fabio Yáñez Romero: *Why LLMs Fail at Knowledge Graph Extraction* & *Modular KG Framework*](failure-modes-of-llm-knowledge-graph-extraction.md) | The cascading pipeline compounding error ($0.9 \times 0.8 \times 0.7 = 50\%$), Asserted vs Augmented graphs, modular `kgb` CLI builder architecture | Both full articles & framework code read |
| P17 | [Fareed Khan: *10M-Document RAG* & *Streaming 2.8T MoE in C on 8GB RAM*](fareed-khan-10m-rag-and-disk-streaming-architectures.md) | "Retrieve, Constrain, Verify, Abstain" (RCVA) 4-stage pipeline; pure C $O(1)$ RAM invariant streaming 1.56 TB checkpoint on 8 GB RAM | Both full deep-dive articles & C code read |
| P18 | [Singhal, Boulahia, Pranit naik: *The Harness is the Product* & *Open Coding Agents*](the-harness-is-the-product-and-open-coding-agents.md) | 7-layer agent harness anatomy, 40% failure engineering, OpenCode human-first terminal diffs, Sakana AI Fugu decision-only routing | All 3 full articles & system reviews read |
| P19 | [Erdogan T, Codebook Fusion, Romero: *Personal Agentic Systems & Neuroplastic Graphs*](personal-agentic-systems-and-neuroplastic-graphs.md) | "LLM is a CPU, not a writer", multi-million doc agentic graph extraction, HOPE/Delta Gradient Descent neuroplastic living memory | All 3 full tutorials & architectural essays read |
...
| **Hierarchical Map Architecture verified** — cite *Your Second Brain Needs a Map* (Romero 2026) for proving Karpathy flat vault breakdown past 1,000 notes and validating 2D path coordinate navigation over vector RAG | `ARCHITECTURE.md` §2.3 / `TAXONOMY.md` |
| **Closed-Ontology & Asserted Graph Mandate** — cite Romero (2026) for quantifying the 50.4% compounding pipeline error trap in unconstrained LLM graph extraction | `specs/RELATION-EXTRACTION.md` / `CANON-005` |
| **Retrieve, Constrain, Verify, Abstain (RCVA) verified** — cite 10M-document benchmark (Khan 2026) for non-neural verification gates and epistemic abstention (<0.4% hallucination) | `specs/RETRIEVAL.md` §9.6-§9.7 / `EPI-007` |
| **Disk-Streaming $O(1)$ RAM Invariant proven** — cite 2.8T MoE C runtime (Khan 2026) for proving 1.56 TB model execution on 8 GB RAM via `mmap` and sequential chunking | `specs/INGEST-ADAPTERS.md` (ADA-008) / `GRAPH-INTELLIGENCE.md` §11.1 |
| **Harness-as-Product & CANON-006 validated** — cite Singhal, Boulahia, and Sakana AI for 7-layer harness primacy, 40% failure engineering, and decision-only routing | `ARCHITECTURE.md` (CANON-006) / `AGENT-SKILLS.md` |
| **CPU Mindset & Neuroplastic Continuums** — cite Erdogan T and Romero for "LLM is a CPU", structured multi-agent moderator trios, and living associative memory continuums | `INGEST-PIPELINE.md` / Plan 98 |
