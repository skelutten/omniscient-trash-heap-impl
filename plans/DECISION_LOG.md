...
| **D113** | Opt-In Parquet/DuckDB Discovery Staging Backend, Schema Evolution & DSCP Commit Protocol: Implemented pluggable staging backend seam (`StagingBackend` ABC) in `trashheap/staging/` supporting both baseline YAML/SQLite (`BaselineStagingBackend`) and opt-in Parquet/DuckDB (`ParquetStagingBackend`). Strictly enforces the Plan 93 Gate: dependency detection (`check_parquet_dependencies`) reports `blocked`/`degraded` if DuckDB/PyArrow are absent and never silently falls back to SQLite while claiming Parquet conformance. Enforces path validation (`_validated_sql_path`) preventing SQL injection and path traversal (`E114`). Enforces type-safe dual-engine schema evolution with SQLite `PRAGMA user_version` migrations 1 -> 2 -> 3 (`ingestion_watermarks`, `promotion_journal`, `used_replay_nonces`, `commit_transactions`) and `TARGET_SCHEMAS` parity for `concept_proposals`, `ambiguous_scope`, and `rejected_low_quality` per `specs/INGEST-STAGING.md` §7.2, emitting `state/schema_version.json`. Implements Durable Staged Commit Protocol (`commit_staged_parquet` per `specs/INGEST-ADAPTERS.md` §3.3) with atomic merge, `os.fsync` durability, and idempotency (same hash = no-op, divergent hash = `DSCPIntegrityError` `E114`). Implements deterministic crash recovery (`recover_pending_dscp_transactions`) covering CP-1, CP-2/CP-3, CP-4/CP-5, and CP-6. Implements atomic manifest emission (`staging/discovery/state/manifest.json`), dual-backend equivalence verification (`verify_backend_equivalence`), and synchronization (`sync_baseline_to_parquet`). Added CLI commands `trashheap staging status|sync|verify-equivalence|manifest|recover`. Verified by 12 tests in `tests/test_parquet_staging.py` and gate `tools/check.sh` (130 passing tests). | `trashheap/staging/`, `tests/test_parquet_staging.py`, `plans/93-OPT-IN-PARQUET-STAGING.md`, `specs/INGEST-STAGING.md` §7, `specs/INGEST-ADAPTERS.md` §3.3 | Executed & Tested |
| **D114** | CLI Ergonomics, Full-Content Access, Root README & Migration Retrieval Specification: Hardened specifications and plans with operational ergonomics derived from end-to-end execution. Added non-truncating `trashheap show <node_id|file_path>` CLI command and `/show` Agent Skill (`specs/AGENT-SKILLS.md` §3.6, `trashheap/skills.py`). Enhanced `trashheap query` with `--include-drafts`, `--include-deprecated`, and `--include-body` flags; documented that `body_excerpt` in Evidence Bundles is bounded ($\le 250$ chars) for LLM context budget while `"path"` provides direct filesystem location (`specs/RETRIEVAL.md` §9.5.1). Updated `plans/01-REPOSITORY-FOUNDATION.md` to require root `README.md` user documentation covering `uv` workflows, datalayout, and storage boundaries (`fixtures/canonical/` vs external wikis). Updated `plans/02-DETERMINISTIC-CORE.md` (WP 10) to include `show` and draft/full-body query flags. Updated `plans/60-OLD-WIKI-MIGRATION.md` to document post-migration retrieval semantics (`--corpus-root <target>` and `--include-drafts` for legacy articles imported as `status: draft` per Rule 10). Emitted root `README.md`. | `specs/RETRIEVAL.md`, `specs/AGENT-SKILLS.md`, `plans/01-REPOSITORY-FOUNDATION.md`, `plans/02-DETERMINISTIC-CORE.md`, `plans/60-OLD-WIKI-MIGRATION.md`, `README.md` | Executed & Tested |
...
| D113 Opt-In Parquet/DuckDB Discovery Staging & DSCP Protocol | Empirical verification: 12 tests in `tests/test_parquet_staging.py`, clean `tools/check.sh` gate (130 passing tests). | Antigravity Agent | 2026-09-07 |
| D114 CLI Ergonomics, Full-Content Access, Root README & Migration Retrieval Specification | Empirical verification: passing tests in `tests/test_retrieval.py`, `tests/test_agent_skills.py`, and gate `tools/check.sh` (130 passing tests). | Antigravity Agent | 2026-09-07 |

This sign-off covers the **decision content** of D79–D95 and runtime foundation D100–D114.
...
2. The next free identifier is **D115**. Do not reuse or backfill gaps in §1.
